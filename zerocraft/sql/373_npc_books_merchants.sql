-- ZeroCraft: NPC Books (v2: Dungeon Elites trimmed to the 12 toughest books, so Merchants fit). Every NPC you can summon, bundled 14 to a book plus a Flight Master in every book.
...
INSERT INTO zc_cand (catord, cat, kind, entry, name)
SELECT 5, 'Dungeon Elites', 823, MIN(g.entry), ct.name FROM zerocraft_guard_pool g JOIN creature_template ct ON ct.entry = g.entry WHERE g.kind = 'dungeon' GROUP BY ct.name ORDER BY MAX(ct.maxlevel) DESC, MAX(ct.`rank`) DESC, ct.name LIMIT 168;
INSERT INTO zc_cand (catord, cat, kind, entry, name)
...
WHERE (ct.npcflag & 128) <> 0 AND ct.`rank` <> 3 AND ct.entry NOT IN (4949,10181,2425,3057,2784,7999,4968,29611,16802,17468,10182,1748,25237,7937,3516,10540) AND (ct.npcflag & (0x1000|0x10000|0x20000|0x200000|0x400000|0x2000)) = 0 GROUP BY ct.name ORDER BY CRC32(ct.name) LIMIT 420;
...
SELECT COUNT(*) AS merchant_books FROM zerocraft_nbook WHERE name LIKE 'NPC Book: Merchants%';
