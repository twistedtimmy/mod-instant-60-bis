...
SELECT i.item,
       ...
       LEFT(CONCAT('NPC Book: ', bk.loc, IF(lc.books > 1, CONCAT(' ', COALESCE(ELT(bk.b + 1, 'I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI','XVII','XVIII','XIX','XX'), CAST(bk.b + 1 AS CHAR))), '')), 80),
       'Calls NPCs from this place. They arrive right in front of you.'
FROM zc_books bk JOIN zc_ids i ON i.n = bk.n JOIN zc_loccount lc ON lc.loc = bk.loc JOIN zc_bookq bq ON bq.loc = bk.loc AND bq.b = bk.b;
