-- ZeroCraft: Frostmourne from the Lich King, Sulfuras from Ragnaros, companion pets from every raid boss.
USE acore_world;
...
UPDATE item_template SET
  ...
  StatsCount = 5,
  stat_type1 = 4,  stat_value1 = 45,    -- Strength
  stat_type2 = 7,  stat_value2 = 45,    -- Stamina
  stat_type3 = 32, stat_value3 = 28,    -- Critical strike rating
  stat_type4 = 31, stat_value4 = 20,    -- Hit rating
  stat_type5 = 44, stat_value5 = 30,    -- Armor penetration rating
  ...
  -- Chance on hit: Deathfrost (frost damage, slows the target's attacks and casting)
  ...
  -- Chance on hit: Lifesteal (drains the target's health into you)
  ...
  -- Use: Lich Slap (frost damage + knockback), 1 minute cooldown
...
SELECT 911070, p.entry, 0, ROUND(p.w * 100 / t.total, 3), 0, 1, 1, 1, 1, CONCAT('ZeroCraft pet - ', p.name)
FROM zc_pets p JOIN (SELECT SUM(w) AS total FROM zc_pets) t;
