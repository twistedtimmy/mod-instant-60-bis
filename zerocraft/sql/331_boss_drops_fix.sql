-- ZeroCraft fix: the creature table here calls its column 'id' (not 'id1'), so the boss lists failed.
USE acore_world;
...
SELECT ct.lootid FROM creature_template ct JOIN zc_spawned s ON s.entry = ct.entry WHERE ct.rank = 3 AND ct.lootid > 0;
INSERT IGNORE INTO zc_raidboss
...
SELECT 'mount reset table ready' AS status;
